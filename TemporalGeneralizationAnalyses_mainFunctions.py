...
import pandas as pd
from EEG_auxiliary_module_guggenmos import run_eeg_svm_guggenmos_tempGen #, run_eeg_distance_guggenmos
import pdb
import random
...
def getandsaveResults(subj,X,Y,condition,event,times,timebin, figNum, n_permutation=1, n_jobs=-1,jitCorrect = None):
 ### subj: Subject Number
 ### X: Selected epochs
 ### Y: Labels
 ...
 ### jitCorrect: Creates different save folders depending on whether jitter correction has been applied or not.
        ...
        np.shape(X)
        acc, duration = run_eeg_svm_guggenmos_tempGen(X,Y,n_pseudo = 5,n_perm=n_permutation,n_jobs=n_jobs)
        ...
        savefile  = 'tempGen_binary_'+condition+'_'+event+'_'+str(timebin)+'ms_v2_reref.npy'
        savepath1 = op.join(dataPath,'Temporal Generalization')
...
def runClassification (subj,condition,timebin = 0,event = 'Stimulus',n_permutation=0,n_jobs=-1,jitCorrect = None):
 ...
 ### jitCorrect: Whether to use jitter-corrected files or not (Default = not).
    ...
    if jitCorrect == None:
        ...
        jitCorrectDataFile = op.join(dataPath,'stimJitterCorrected50_reref.npy')
    ...
    if condition == 'Faces'or condition == 'All':
        ...
        figNum = getandsaveResults(subj,X,Y,'Faces',event,times,timebin, figNum,n_permutation=n_permutation,n_jobs=n_jobs,jitCorrect= jitCorrect)
        del X,Y
    ...
    if condition == 'Colors' or condition == 'All':
        ...
        Y = np.int32(Y[0])
        # ### Running discrimination analysis
        figNum = getandsaveResults(subj,X,Y,'Colors',event,times,timebin, figNum,n_permutation=n_permutation,n_jobs=n_jobs,jitCorrect=jitCorrect)
        del X,Y
    
    #%%####------------------------- Values -----------------------------------------
    if condition == 'Values' or condition == 'All':
        # Define basic condition information
        newValues = [0]*5
        ...
        for v in range(len(newValues)):
            ...
            if v>0:
                ...
                X = valueAvg
        # ### Running discrimination analysis
        figNum = getandsaveResults(subj,X,Y,'Value-2',event,times,timebin, figNum,n_permutation=n_permutation,n_jobs=n_jobs,jitCorrect=jitCorrect)
    ...
    if condition == 'Value_binary' or condition == 'All':
        ...
        # ### Running discrimination analysis
        figNum = getandsaveResults(subj,X,Y,'Value_binary',event,times,timebin, figNum,n_permutation=n_permutation,n_jobs=n_jobs,jitCorrect=jitCorrect)
    ...
    if condition == 'Value_absMag' or condition == 'All':
        ...
        print('Hello')
        # ### Running discrimination analysis
        figNum = getandsaveResults(subj,X,Y,'Value_absMag',event,times,timebin, figNum,n_permutation=n_permutation,n_jobs=n_jobs,jitCorrect=jitCorrect)
    ...
    if condition == 'FaceValues' or condition == 'All':
        ...
        ttime = np.linspace(times[0],times[1],AllEpochs.shape[2])
        valList = []
        #plt.title(epochs.ch_names[chan]+ ' Face Values')
        ...
        figNum = getandsaveResults(subj,X,Y,'FaceValues',event,times,timebin, figNum,n_permutation=n_permutation,n_jobs=n_jobs,jitCorrect=jitCorrect)
        del X,Y    
    #%%####------------------------- Color Values -----------------------------------------
    if condition == 'ColorValues' or condition == 'All':
        ...
        # ### Running discrimination analysis
        figNum = getandsaveResults(subj,X,Y,'ColorValues',event,times,timebin, figNum,n_permutation=n_permutation,n_jobs=n_jobs,jitCorrect=jitCorrect)
    ...
    if condition == 'Response' or condition == 'All':
        ...
        # ### Running discrimination analysis
        figNum = getandsaveResults(subj,X,Y,'Response-0',event,times,timebin, figNum,n_permutation=n_permutation,n_jobs=n_jobs,jitCorrect=jitCorrect)
        del X,Y   
